        ...
        try:
            ...
            if row:
                ...
                conn.execute("""
                    ...
                    series_index=?, collection_path=?, category=?, format=?, file_size=?,
                    has_cover=?, page_count=?, description=?, modified_at=CURRENT_TIMESTAMP
                    WHERE id=?
                """, (meta["title"], meta["author"], meta["genre"],
                      meta["series"], meta["series_index"], meta.get("collection_path", ""),
                ...
                conn.execute("""
                    ...
                    series_index, collection_path, category, format, file_size, has_cover, page_count, description)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (meta["path"], meta["filename"], meta["title"],
                      meta["author"], meta["genre"], meta["series"],
                      meta["series_index"], meta.get("collection_path", ""),
                      meta["category"], meta["format"],
